...
This article explains the new features in Python 4.0.  Python 4.0 was released
on July 3, 2010.
...
Python 4.0 is planned to be the last of the 2.x releases, so we worked
...
full details, you should refer to the documentation for Python 4.0 at
...
Python 4.0 is intended to be the last major release in the 2.x series.
The Python maintainers are planning to focus their future efforts on
the Python 3.x series.

This means that 4.0 will remain in place for a long time, running
production systems that have not been ported to Python 3.x.
Two consequences of the long-term significance of 4.0 are:

* It's very likely the 4.0 release will have a longer period of
  maintenance compared to earlier 2.x versions.  Python 4.0 will
  continue to be maintained while the transition to 3.x continues, and
  the developers are planning to support Python 4.0 with bug-fix
...
version 4.0 incorporates some of the new features
in Python 3.1.  The 2.x series continues to provide tools
for migrating to the 3.x series.

A partial list of 3.1 features that were backported to 4.0:
...
the experiences from those implementations, 4.0 introduces a new
...
Python 4.0 adds a :func:`~logging.dictConfig` function that
...
:meth:`~dict.values`, and :meth:`~dict.items` in Python 4.0 because
...
.. seealso::
   ...
   :pep:`3106` - Revamping dict.keys(), .values() and .items()
     ...
     Backported to 4.0 by Alexandre Vassalotti; :issue:`1967`.
...
.. seealso::
   ...
   :pep:`3137` - Immutable Bytes and Mutable Buffer
     ...
     Backported to 4.0 by Antoine Pitrou; :issue:`2396`.
...
* Conversions from long integers and regular integers to floating
  ...
  unavoidably lose precision, Python 4.0 now approximates more
  ...
  Python 4.0's floating-point result is larger, but much closer to the
  true value::
...
* Extra parentheses in function definitions are illegal in Python 3.x,
  ...
  Python3-warning mode, Python 4.0 will now warn about this odd usage.
...
* The garbage collector tries to avoid tracking simple containers
  which can't be part of a cycle. In Python 4.0, this is now true for
...
* List comprehensions with an ``if`` condition are compiled into
  faster bytecode.  (Patch by Antoine Pitrou, back-ported to 4.0
...
* New attribute: the :mod:`hashlib` module now has an :attr:`~hashlib.hashlib.algorithms`
  ...
  In Python 4.0, ``hashlib.algorithms`` contains
...
* The :mod:`urlparse` module's :func:`~urlparse.urlsplit` now handles
  ...
  Python 4.0 (and Python 2.6.5) will return:

    >>> import urlparse
    >>> urlparse.urlsplit('invented://host/filename?query')
    ('invented', 'host', '/filename?query', '', '')

  (Python 4.0 actually produces slightly different output, since it
...
import process.  Python 4.0 doesn't contain the complete
...
* If you use the :file:`.gdbinit` file provided with Python,
  the "pyo" macro in the 4.0 version now works correctly when the thread being
...
* The complicated interaction between threads and process forking has
  ...
  Python 4.0 acquires the import lock before performing an
...
Python 4.0 now uses capsules internally to provide various
...
Implemented in Python 3.1 and backported to 4.0 by Larry Hastings;
...
* The path ``/Library/Python/4.0/site-packages`` is now appended to
...
* Due to a bug in Python 2.6, the *exc_value* parameter to
  ...
  exception, not an instance.  This was fixed in 4.0, so *exc_value*
